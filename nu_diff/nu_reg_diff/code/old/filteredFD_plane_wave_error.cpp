...
void hdaf_lp_kernel( double a, double b, int n, int m, double gamma, double *& ker, int &w )
{
    double h = (b-a)/n;
    ...
    w = (int)ceil(hdaf_truncate_point (1E-16, 2E-16, m, sigma, 0 )/h);
        
    assert(ker == 0);
    ker = ml_alloc<double > (2*w+1);
        
    mp_real H = h;
    mp_real s = H*H/((mp_2*sigma)*sigma);
        
    for (int j=-w; j<=w; j++ )
        ker[j+w] = dble( exp(-s*(j*j)) *P( dble((H*j)/(mp_sqrt2*sigma))) /(mp_sqrt2*sigma) );
}
...
int main()
{
   ...
   for (int g=1; g<=10; g++)
   {
        ...
        double **error = ml_alloc<double> ( n/2-1, n_D );
        double * ker=0;
        int w = 0;
        
        hdaf_lp_kernel( a, b, n, 30, 0.1*g, ker, w );
        cout << "hdaf width =  " << w << endl;
        ...
        for ( int k_ = 1; k_<n/2; k_++ )
        {
            ...
            for ( int j=0; j<n_D; j++ )
            {
                if (j == 0)
                    ...
                    FD (1, m, a, b, n, G[j-1], temp );
                
                double * p = &(ker[w]);
                periodic_convolve( n, temp, G[j], w, p, h );
                
            //    FD (1, m, a, b, n, G[j-1], G[j] );
            }
            
            for ( int j=0; j<n_D; j++ )
                error[k_-1][j] = l2_error( D[j], G[j], n, a, b );
   ...
   /*
    {
        // verify frequency response of hdaf
        ...
        for ( int k_ = 0; k_<n/2; k_++ )
        {
            double k = (_2pi*k_)/(b-a);
            
            double h = (b-a)/n;
            double * F = ml_alloc<double> ( n );
            
            for ( int i=0; i<n; i++ )
                F[i] = cos((h*i)*k);
            
            double *G = ml_alloc<double> ( n );
            
            hdaf_lp( a, b, n, m, g, F, G );
            
            cout << k_ << "\t" << rms( G, n, a, b )/rms( F, n, a, b ) << "\t" <<  l2_error( F,G,n,a,b )  <<  endl;
        }
